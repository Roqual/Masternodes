...
DAEMON_DOWNLOAD_EXTRACT () {
  ...
  do
    ...
    then
      echo "Setting executable bit for ${DAEMON_BIN}"
    ...
    then
    echo "Setting executable bit for ${CONTROLLER_BIN}"
...
DAEMON_DOWNLOAD_SUPER () {
  ...
  then
    ...
    if [[ -f /var/multi-masternode-data/"${PROJECT_DIR}"/src/"${CONTROLLER_BIN}" ]] && [[ $( echo "${CONTROLLER_BIN}" | grep -cE "cli$" ) -gt 0 ]]
    then
    ...
    LINES=$( echo "${DOWNLOADS}" | sed '/^\s*$/d' | wc -l )
    if [[ "${LINES}" -eq 0 ]]
    then
      echo "ERROR!"
    elif [[ "${LINES}" -eq 1 ]]
    ...
    if [[ $( echo "${DAEMON_DOWNLOAD_URL}" | sed '/^\s*$/d' | wc -l ) -gt 1 ]]
    then
  ...
  then
    RELEASES=$( wget -4qO- -o- "https://api.github.com/repos/${REPO}/releases" )
    DAEMON_DOWNLOAD_URL=$( echo "${RELEASES}" | jq -r '.[].assets[].browser_download_url' )
    DAEMON_DOWNLOAD_EXTRACT "${PROJECT_DIR}" "${DAEMON_BIN}" "${CONTROLLER_BIN}" "${DAEMON_DOWNLOAD_URL}"
  fi
}
...
GET_FREE_IPV46() {
  ...
  do
    if [[ "$( sudo timeout 0.2s netcat -p "${PORT_TO_TEST}" -l "${PRIVIPADDRESS}" 2>&1 | grep -ci 'in use' )" -gt 0 ]]
    then
      continue
    fi
...
FIND_OPEN_PORT_IPV46 () {
  ...
  RUNNING_DAEMONS=$( echo "${RUNNING_DAEMONS}" | sort -u | sed '/^\s*$/d' )
  ...
  then
    ...
    PUBLIC_IPV4=$( GET_PUBLIC_IPV46 "${IPV4_ADDRESSES}"  )
    FREE_IPV4=$( GET_FREE_IPV46 "${PUBLIC_IPV4}" "${PORT_TO_TEST}" | sort -V )
  ...
  then
    ...
    PUBLIC_IPV6=$( GET_PUBLIC_IPV46 "${IPV6_ADDRESSES}"  )
    FREE_IPV6=$( GET_FREE_IPV46 "${PUBLIC_IPV6}" "${PORT_TO_TEST}" | sort -V )
...
#   fi
  ...
  then
    ...
    then
      ...
      then
        FINAL_IP_ADDRESS=$( echo "${RUNNING_IPV4}" | grep -v '0.0.0.0' | cut -d ':' -f1 | awk '{print $2}' | sort -hr | uniq -c | sort -hr | tail -n 1 | awk '{print $2}' )
        if [[ -z "${FINAL_IP_ADDRESS}" ]]
        then
          PUBIPADDRESS=$( echo "${PUBLIC_IPV4}" | head -n 1 | awk '{print $2}')
          PRIVIPADDRESS=$( echo "${PUBLIC_IPV4}" | head -n 1 | awk '{print $3}')
        else
          PUBIPADDRESS=$( echo "${PUBLIC_IPV4}" | grep "${FINAL_IP_ADDRESS}" | cut -d '/' -f1 | awk '{print $2}' )
        ...
        do
          ...
          if [[ $( sudo ss -lpn 2>/dev/null | grep -c ":${PORT_TO_USE}[[:space:]]" ) -eq 0 ]] && [[ $( sudo timeout 0.2s netcat -p "${PORT_TO_USE}" -l "${PRIVIPADDRESS}" 2>&1 | grep -ci 'in use' ) -eq 0 ]]
          then
            break
          fi
        done
  ...
  then
    ...
    then
      ...
      PRIVIPADDRESS=$( echo "${IPV6_ADDRESSES}" | grep "${FINAL_IP_ADDRESS}" | cut -d '/' -f1 | awk '{print $2}' )
#       PRIVIPADDRESS=${FINAL_IP_ADDRESS}
      ...
      IPV6_BASE=$( echo "${IPV6_ADDRESSES}" | awk '{print $2}' | grep "${IPV6_BASE}" | grep -v '/128' | tail -n 1 )
      # Get Private IPv6 Address interface name.
      IPV6_INTERFACE=$( sudo ip -o addr show | grep "${IPV6_BASE}"| awk '{print $2}' )
      ...
      NEW_IP=1
      while [[ $( echo "${IPV6_ADDRESSES}" | grep -cF "${IPV6_BASE_START}" ) -gt 0 ]]
      do
        sleep 0.1
        LAST_HEX=$( echo "${IPV6_BASE_START}" | grep -o ':[0-9a-f]*$' | cut -d ':' -f2 | tr '[:lower:]' '[:upper:]' )
        LAST_HEX=$( echo "obase=ibase=16; ${LAST_HEX} + 1" | bc )
        IPV6_BASE_START=$( echo "${IPV6_BASE_START}" | sed "s/\\:\\([a-f0-9]*\\)$/\\:${LAST_HEX}/g" | tr '[:upper:]' '[:lower:]' )

        if [[ "${IPV6_BASE_START}" == "${IPV6_BASE_END}" ]]
        then
          NEXT_DAEMON='N'
          NEW_IP=0
          IPV6_BASE_START=''
      ...
      if [[ "${NEW_IP}" -eq 1 ]]
      then
        ip addr add "${IPV6_BASE_START}/${IPV6_BASE_BITS}" dev "${IPV6_INTERFACE}"
        sleep 1
        echo "Restarting the networking service"
        sudo service networking restart 2>/dev/null
        sudo netplan apply 2>/dev/null
      fi
      PUBIPADDRESS=${IPV6_BASE_START}
      PRIVIPADDRESS=${IPV6_BASE_START}
    fi
  fi
...
(renice 15 $BASHPID
  ...
  then
    WAIT_FOR_APT_GET
    nice -n 15 sudo DEBIAN_FRONTEND=noninteractive apt-get install -yq bc
  fi
...
_masternode_dameon_2 () {
  ...
  if [ "${ARG9}" == "masternode" ] && [ "${_MASTERNODE_CALLER}" != "masternode" ]
  then
    ARG9=${_MASTERNODE_CALLER}
  fi
  if [[ "${ARG9}" == mn* ]] && [ "${_MASTERNODE_PREFIX}" != "mn" ]
  then
    ARG9="${_MASTERNODE_PREFIX}${ARG9:2}"
  fi
  ...
  then
    ...
    do
      ...
      sleep 0.3
      if [[ $( "${1}" failure_after_start | sed '/^\s*$/d' | wc -l ) -gt 0 ]]
      then
        break;
      fi
    done
  ...
  then
    if [[ $( "${1}" blockcheck 2>/dev/null | sed '/^\s*$/d' | wc -l ) -gt 1 ]]
    then
      "${1}" stop
      "${1}" remove_addnode
  ...
  then
    if [[ $( "${1}" system_log | grep -Fi ": error: couldn't connect to server" | tail -n 5 | sed '/^\s*$/d' | wc -l ) -lt 5 ]]
    then
      return
    fi
  ...
  then
    ...
    then
      ...
      OUTPUTIDX_RAW=$( wget -4qO- -T 15 -t 2 -o- "${3}api/getrawtransaction?txid=${TXID}&decrypt=1" "${TEMP_VAR_C}"  )
      sleep 1
      MN_WALLET_BALANCE=$( wget -4qO- -T 15 -t 2 -o- "${3}ext/getaddress/${MN_WALLET_ADDR}" "${TEMP_VAR_C}" | jq -r '.balance' )
      sleep 1
    fi
  ...
  then
    ...
    then
      ...
      JSON_STRING=$( "${USER_HOME_DIR}/.local/bin/${2}" "-datadir=${DIR}/" "${ARG9}" ${10} ${11} ${12} ${13} ${14} ${15} ${16} ${17} 2>&1 )
    fi
...
UPDATE_DAEMON_ADD_CRON () {
  ...
  do
    ...
    if [[ $( "${MN_USRNAME}" blockcheck 2>/dev/null | sed '/^\s*$/d' | wc -l ) -gt 1 ]]
    then
      "${MN_USRNAME}" stop
...
# then
#   echo "Testing IPv4"
#   IPV4_WORKS=$( timeout 3s wget -4qO- -T 3 -t 2 -o- icanhazip.com )
#   if [[ -z "${IPV4_WORKS}" ]]
#   then
#     echo "Restarting the networking service"
#     sudo service networking restart 2>/dev/null
...
#   if [[ -z "${IPV4_WORKS}" ]]
#   then
#     IPV4_WORKS=$( timeout 3s wget -4qO- -T 3 -t 2 -o- v4.ident.me )
#   fi
#   if [[ -z "${IPV4_WORKS}" ]]
#   then
#     IPV4_WORKS=$( timeout 3s wget -4qO- -T 3 -t 2 -o- ipinfo.io/ip )
#   fi
#   if [[ -z "${IPV4_WORKS}" ]]
#   then
#     echo
#     echo "Looks like IPv4 isn't working correctly."
#     echo "You might want to restart the server."
#     echo
#     sleep 3
#   fi
# fi
...
# then
#   echo "Testing IPv6"
#   IPV6_WORKS=$( timeout 3s wget -6qO- -T 3 -t 2 -o- icanhazip.com )
#   if [[ -z "${IPV6_WORKS}" ]]
#   then
#     echo "Restarting the networking service"
#     sudo service networking restart 2>/dev/null
...
#   if [[ -z "${IPV6_WORKS}" ]]
#   then
#     IPV6_WORKS=$( timeout 3s wget -6qO- -T 3 -t 2 -o- v6.ident.me
#   fi
#   if [[ -z "${IPV6_WORKS}" ]]
#   then
#     IPV6_WORKS=$( timeout 3s wget -6qO- -T 3 -t 2 -o- ipv6.whatip.me | hxnormalize -x | hxselect -i -c 'body p' )
#   fi
#   if [[ -z "${IPV6_WORKS}" ]]
#   then
#     echo
#     echo "Looks like IPv6 isn't working correctly."
#     echo "You might want to restart the server."
#     echo
#     sleep 3
#   fi
...
do
  ...
  if [[ $( sudo ss -lpn 2>/dev/null | grep -c ":${PORTA}[[:space:]]" ) -eq 0 ]] && [[ $( sudo timeout 0.2s netcat -p "${PORTA}" -l "${PRIVIPADDRESS}" 2>&1 | grep -ci 'in use' ) -eq 0 ]]
  then
    break
  fi
done
...
then
  ...
  do
    ...
    OUTPUTIDX_COUNT=$( echo "${OUTPUTIDX_WEB}" | sed '/^\s*$/d' | wc -l )
    ...
    then
      OUTPUTIDX=$( echo "${OUTPUTIDX_WEB}" | sed '/^\s*$/d' )
    fi
...
do
  ...
  if [[ $( "${GOOD_MN_USRNAME}" blockcheck 2>/dev/null | sed '/^\s*$/d' | wc -l ) -eq 1 ]]
  then
    echo " It is good!"
    if [[ "${FAST_SYNC}" -eq 1 ]]
    then
...
if [[ "$( sudo ss -lpn 2>/dev/null | grep -c "${BIND}[[:space:]]" )" -gt 0 ]] || [[ $( sudo timeout 0.2s netcat -p "${PORTB}" -l "${PRIVIPADDRESS}" 2>&1 | grep -ci 'in use' ) -gt 0 ]]
then
  echo
...
if [[ "$( sudo ss -lpn 2>/dev/null | grep -c ":${PORTA}[[:space:]]" )" -gt 0 ]] || [[ $( sudo timeout 0.2s netcat -p "${PORTA}" -l "${PRIVIPADDRESS}" 2>&1 | grep -ci 'in use' ) -gt 0 ]]
then
  echo "Searching for an unused port for rpc"
  while :
  do
...
then
  ...
  do
    ...
    if [[ $( "${USRNAME}" privkey 2>/dev/null | sed '/^\s*$/d' | wc -l ) -eq 0 ]]
    then
      break
    fi
